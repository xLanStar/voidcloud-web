        ...
        children: [
            {
                ...
                beforeEnter: async (to) => {
                    ...
                    if (!folder) return StorageHelper.token ? { name: 'VoidCloud-Folder', params: { folder: ['drive'] }} : { name: 'Login' };
                    const path = folder && folder.join("/") || "/drive/";
                    StorageHelper.client.setHeaders({
                        ...StorageHelper.client.getHeaders(),
                        ...(to.query.folder) && { Folder: to.query.folder }
                    })
                    const exists = await StorageHelper.client.exists(path).catch(() => {});
                    if(!exists) return path !== '/drive/' && StorageHelper.token ? { name: 'VoidCloud' } : { name: 'Login' };
                },
...
router.beforeEach((to) => {
    ...
    // 跳轉登入頁面時，若已登入則跳轉到首頁
    if (to.name == 'Login' && state.user) return { name: 'VoidCloud-Folder', params: { folder: ['drive'] } };
