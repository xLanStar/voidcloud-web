...
export const viewFolder = async (filepath) => {
  ...
  await StorageHelper.client
    .getDirectoryContents(filepath)
    .then((data) => {
      viewingFolder.files = data
        .slice(1)
        .map((file) => {
          return { ...file, isDir: file.type === "directory" };
        })
        .sort((a, b) => a.isDir && -1);
...
const _viewFile = (index) => {
  ...
  viewingFile.link = `${StorageHelper.client.getFileDownloadLink(
    viewingFile.filepath
  )}?Authorization=${localStorage.getItem("token")}${
    viewingFolder.shareFolder ? `&folder=${viewingFolder.shareFolder}` : ""
  }`;
...
export const viewPrevious = () => {
  let index =
    (viewingFile.index - 1 + viewingFolder.files.length) %
    viewingFolder.files.length;
  while (getViewType(viewingFolder.files[index]) !== viewingFile.viewType)
    index =
      (index - 1 + viewingFolder.files.length) % viewingFolder.files.length;
  _viewFile(index);
};
