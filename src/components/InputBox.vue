...
<template>
	<div class="inputbox" :class="validateStatus">
		<SvgIcon v-if="icon" :icon="icon" :color="focus ? SVG_ICON_COLOR_PRIMARY : SVG_ICON_COLOR_SECONDARY" @click="e => emits('clickIcon', e)" />
		<input class="input" :type="isHidden ? type : 'text'"
			:maxlength="maxlength" :min="min" :max="max" :value="value" :placeholder="placeholder"
			@focus="focus = true; focusOnce = true" @blur="focus = false" />
		<SvgIcon v-if="type === 'password'" :icon="isHidden ? 'EyeClosed' : 'Eye'" :color="focus ? SVG_ICON_COLOR_PRIMARY : SVG_ICON_COLOR_SECONDARY" @click="handleHide" />
		<SvgIcon v-if="validateStatus" :icon="statusIconMap[validateStatus]" :color="statusColorMap[validateStatus]" />
	</div>
</template>
...
.input {
	...
	outline: 0;
	box-shadow: none;
	transition: background-color .2s ease-out, box-shadow .2s ease-in;
	padding: 0 1rem;
	text-align: left;
