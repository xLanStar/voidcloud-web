...
const fileMenu = [
  {
    ...
    text: "共用",
    isActive:
      startSelectedIndex.value == endSelectedIndex.value &&
...
const select = (left, top, right, bottom) => {
...
onBeforeRouteUpdate(
  (to) => StorageHelper.client && viewFolder(to.params.folder.join("/"))
);
  ...
  <div v-if="viewingFolder.accessable" id="drive-folder">
    <div
      ...
      @contextmenu.stop.prevent="
        (e) => showMenu(backMenu, e.clientX, e.clientY)
      "
    >
      <div id="explorer-header">
        <div id="folder-path">
          <span
            @click="
              router.push({
                ...
                params: { folder: route.params.folder.slice(0, 1) },
          ...
          <span
            v-for="(folder, idx) in route.params.folder.slice(1)"
            @click="
              router.push({
                name: 'VoidCloud-Folder',
                params: { folder: route.params.folder.slice(0, idx + 2) },
                query: { folder: route.query.folder },
              })
            "
          >
      ...
      <div
        id="folder-explorer-content"
        @mousedown="onMouseDownExplorer"
        @mouseup="onMouseUpExplorer"
      ...
      >
        ...
        <div id="file-container-wrapper">
          <div
            ref="fileContainer"
            :id="grid ? 'file-grid' : 'file-list'"
            class="file-container"
          >
            <FileItem
              v-for="(file, idx) in viewingFolder.files"
              :key="idx"
              :file="file"
              :grid="grid"
              ...
              @dblclick="
                file.isDir
                  ? router.push({
                      name: 'VoidCloud-Folder',
                      params: {
                        folder: [...route.params.folder, file.basename],
                      },
                      query: { folder: route.query.folder },
                    })
                  : ViewFile(idx)
